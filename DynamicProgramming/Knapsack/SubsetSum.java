...
When the number of elements(n) is 0 (i == 0), and the sum is greater than 0 (j > 0), there is no subset possible, so set dp[0][j] = false.

[4] this code checks whether the current element arr[i-1] is less than or equal to the target sum j.
If it is, then we have two choices:
...
[5] If arr[i-1] is greater than j, it means that the current element cannot be included in the subset to achieve the sum of j.
